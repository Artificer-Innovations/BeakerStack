...
describe('initObservability (web)', () => {
  ...
  it('skips init when getClient already returns a client', () => {
    vi.mocked(SentryMock.getClient).mockReturnValue({} as any);
    initObservability(config);
    expect(SentryMock.init).not.toHaveBeenCalled();
  });

  it('beforeSend strips ip_address when captureIp is not set', () => {
    initObservability(config);
  ...
  it('beforeSend preserves ip_address when captureIp is true', () => {
    initObservability({ ...config, pii: { captureIp: true } });
    const { beforeSend } = vi.mocked(SentryMock.init).mock.calls[0][0] as any;
  ...
  it('beforeSend returns event unchanged when no user', () => {
    initObservability(config);
    const { beforeSend } = vi.mocked(SentryMock.init).mock.calls[0][0] as any;
