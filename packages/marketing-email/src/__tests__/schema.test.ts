...
describe('defineMarketingEmailConfig', () => {
  ...
  it('accepts namespace with digits and hyphens', () => {
    expect(() =>
      defineMarketingEmailConfig({ provider: 'kit', namespace: 'my-app-2', kitFormId: '123' })
    ).not.toThrow();
  });

  it('rejects namespace with uppercase letters', () => {
    expect(() =>
      defineMarketingEmailConfig({ provider: 'kit', namespace: 'MyApp', kitFormId: '123' })
    ).toThrow('lowercase letters');
  });

  it('rejects namespace with spaces', () => {
    expect(() =>
      defineMarketingEmailConfig({ provider: 'kit', namespace: 'my app', kitFormId: '123' })
    ).toThrow('lowercase letters');
  });

  it('rejects namespace starting with a digit', () => {
    expect(() =>
      defineMarketingEmailConfig({ provider: 'kit', namespace: '1app', kitFormId: '123' })
    ).toThrow('lowercase letters');
  });

  it('rejects namespace with special characters', () => {
    expect(() =>
      defineMarketingEmailConfig({ provider: 'kit', namespace: 'my_app', kitFormId: '123' })
    ).toThrow('lowercase letters');
  });
});
