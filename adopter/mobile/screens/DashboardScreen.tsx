...
const METERS = [METER_AI_SUMMARIZE] as const;

function DemoControlsBlock(): ReactElement | null {
  const demoMode = readBillingDashboardDemoMode();
  const { data: plan, loading: planLoading } = usePlan<typeof billingConfig>();
  const { refreshSubscription } = useBillingContext<typeof billingConfig>();
              ...
              onPress={() =>
                void run(`p-${p.id}`, async () => {
                  const { error: e } = await supabase.rpc(
                    ...
                    {
                      p_product_id: billingConfig.productId,
                      p_plan_id: p.id,
                    }
                  );
                  if (e) throw e;
