...
| `kit-sync`         | **Disabled**                        | Dequeues `marketing_email_sync_queue` and syncs tags/subscribers to Kit API v4 (pg_cron every 5 min).                      |
| `kit-webhook`      | **Disabled**                        | Inbound Kit unsubscribe webhooks (`x-kit-signature` HMAC verification).                                                    |
...
- `KIT_API_KEY` — Kit Creator API v4 key (outbound sync). Set via `npm run setup:kit` → GitHub secret `KIT_API_KEY` (shared across preview/staging/production).
- `KIT_CRON_SECRET` — Bearer token for `kit-sync` (pg_cron + manual bulk-sync). Auto-generated by `setup:kit` if omitted.
- `KIT_WEBHOOK_SECRET` — Kit dashboard webhook signing secret (inbound `kit-webhook`).
...
Per-environment **non-secret** Kit config (`namespace`, `kitFormId`, `tierTagNames`) lives in `marketing_email_settings` via **Admin → Marketing Email Settings** (`/admin/marketing-email/settings`).
